 ...
 * booking.js  (used by booking.html — row/list staff layout)
 *
 * Handles:
 *  - Step navigation (1 → 2 → 3 and back)
 *  - Form validation
 *  - Staff row rendering + selection
...
function validateAndCollectStep1() {
  ...
  if (valid) {
    ...
    state.userInfo.message   = document.getElementById('message').value.trim();
  }
...
/* ============================================================
   8. STEP 2 — STAFF SELECTION (row/list style)
   ============================================================ */
function renderStaffCards() {
  ...
  STAFF.forEach((member) => {
    ...
    row.className = `staff-row${member.highlight ? ' highlight' : ''}`;
    row.setAttribute('role', 'radio');
    row.setAttribute('tabindex', '0');
    row.setAttribute('aria-checked', 'false');
    row.setAttribute('aria-label', `Select ${member.name} – ${member.departments.join(', ')}`);
    ...
    row.innerHTML = `
      ...
      <div class="row-label">
        ...
        <span class="row-depts">${member.departments.join(', ')}</span>
      </div>
    `;

    row.addEventListener('click', () => selectStaff(row, member));
    row.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        selectStaff(row, member);
      }
    });
...
function selectStaff(rowEl, member) {
...
function setupStep2() {
  ...
  if (state.selectedStaff) {
    const prevRow = staffGrid.querySelector(`[data-id="${state.selectedStaff.id}"]`);
    if (prevRow) {
      prevRow.classList.add('selected');
      prevRow.setAttribute('aria-checked', 'true');
      step2Next.disabled = false;
    }
  }
}
