...
async function sendEmail({ to, subject, html, replyTo }) {
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from:     'SPA Ajibade Booking <onboarding@resend.dev>',
      to,
      subject,
      html,
      reply_to: replyTo,
  ...
  if (!response.ok) {
    // Get the actual error details from Resend to see why it failed
    const error = await response.json().catch(() => ({ message: 'Unknown Resend Error' }));
...
if (typeof fetch === 'undefined') {
  console.error("CRITICAL: 'fetch' is not defined. Ensure you are using Node.js v18 or higher.");
}
